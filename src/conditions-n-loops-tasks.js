...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = i + 1; j < n; j += 1) {
      const temp = cmatrix[i][j];
      cmatrix[i][j] = matrix[j][i];
      cmatrix[j][i] = temp;
    }
  }
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    let right = n - 1;
    while (left < right) {
      const temp = cmatrix[i][left];
      cmatrix[i][left] = matrix[i][right];
      cmatrix[i][right] = temp;
      left += 1;
      right -= 1;
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    if (seen.has(current)) {
      ...
      const cycle = i - start;
      const remaining = (iterations - i) % cycle;
